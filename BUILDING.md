...
- `RPP_TYPE_ERASED_OBSERVABLE` - (ON/OFF) force observable to store any `on_subscribe` as `std::function`. As a result it reduces amount of uniq observables, speedups compilation and IDE loading time a lot, but has a major runtime performance hit. Strongly recommended to avoid this option. Enable ONLY when it is not possible to use RPP as is (for example, system has limits over ROM/RAM/CPU). (default OFF)
