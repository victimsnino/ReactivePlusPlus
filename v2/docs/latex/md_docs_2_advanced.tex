...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto}\ s1\ =\ rpp::source::just(1).repeat().subscribe\_on(rpp::schedulers::new\_thread\{\});}
\DoxyCodeLine{\textcolor{keyword}{auto}\ s2\ =\ rpp::source::just(2).repeat().subscribe\_on(rpp::schedulers::new\_thread\{\});}
...
To achieve better performance use {\ttfamily specific\+\_\+observable} while it is possible. Same for the argument of callback (for example, when you use {\ttfamily \doxylink{group__creational__operators_gae68c87e7ae3a505258b733082bf68513}{rpp\+::source\+::create}})\+: use {\ttfamily const auto\&} for subscriber to avoid implicit conversion to dynamic subscriber.
