...
for platform, data in results.groupby("platform"):
    ...
    for name, bench_data in cur_data.groupby("benchmark_name"):
        ...
        print("")
        print("Test Name | Current, ns | Prev, ns | Ratio | RxCpp current, ns | Rpp Type-erased, ns")
        print("--- | --- | --- | --- | --- | ---")
        for _, r in bench_data.iterrows():
            ...
            rpp_type_erased_data = bench_data[(bench_data['benchmark_name']==name) & (bench_data['test_case'] == r['test_case']) & (bench_data['source'] == 'rpp_type_erased')]
            ...
            rpp_type_erased_value = f"{rpp_type_erased_data['value'].values[0] :.2f}ns" if len(rpp_type_erased_data['value'].values) >= 1 else '.'
            print(f"{r['test_case']} | { new_value } | { old_value } | {ratio} | {rxcpp_value} | {rpp_type_erased_value}")
        print("")
        print("</details>")
        print("")
