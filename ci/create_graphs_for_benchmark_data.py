...
for platform, data in results.groupby("platform", sort=False, as_index=False):
    ...
    for i, (name, bench_data) in enumerate(data.groupby("benchmark_name", sort=False, as_index=False)):
        ...
        fig = px.line(bench_data, x="commit", y="value", color="test_case", line_shape='spline', markers=True, hover_data=hover_data, title=name, height=800, labels=labels, line_dash='source', render_mode='svg')
        copy_data = fig["data"]
        for v in copy_data:
