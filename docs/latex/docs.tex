...
\hypertarget{quick_start_autotoc_md17}{}\doxysubsection{Brief workflow}\label{quick_start_autotoc_md17}
For the brief overview of the Reactive pattern read \href{https://reactivex.io/}{\texttt{ https\+://reactivex.\+io/}}.

In short, creation of programs with help of Reactive\+Plus\+Plus split into several parts\+:\hypertarget{quick_start_autotoc_md18}{}\doxysubsubsection{1) Define observables}\label{quick_start_autotoc_md18}
Observables are sources of your future streams. First of all you need to create some observable which emits values. You can select from some \href{https://victimsnino.github.io/ReactivePlusPlus/docs/html/group__observables.html}{\texttt{ predefined}} or built your own.

For example, 
\begin{DoxyCode}{0}
\DoxyCodeLine{\mbox{\hyperlink{group__creational__operators_gaf574d9dd582ee9b59553e3705e8793cd}{rpp::source::from\_callable}}(\&::getchar)}
\end{DoxyCode}


observable which emits one char from {\ttfamily cin} via invoking of provided function once after subscription

Action inside observable happens O\+N\+LY after subscription on this observable and O\+N\+LY for provided subscriber/observer. It means, that you can subscribe on the same observable multiple times!\hypertarget{quick_start_autotoc_md19}{}\doxysubsubsection{2) Chain observable}\label{quick_start_autotoc_md19}
...
\DoxyCodeLine{.map(\&::toupper)}
\end{DoxyCode}
\hypertarget{quick_start_autotoc_md20}{}\doxysubsubsection{3) Subscription}\label{quick_start_autotoc_md20}
...
\begin{DoxyItemize}
\item (optional) subscription
\item (optional) subscription, {\ttfamily on\+\_\+next}
\item (optional) subscription, {\ttfamily on\+\_\+next}, {\ttfamily on\+\_\+error}
\item (optional) subscription, {\ttfamily on\+\_\+next}, {\ttfamily on\+\_\+completed}
\item (optional) subscription, {\ttfamily on\+\_\+next}, {\ttfamily on\+\_\+error} {\ttfamily on\+\_\+completed}
\item (optional) subscription, observer 
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{auto} s1 = \mbox{\hyperlink{group__creational__operators_gae8ac01568a0e057e97cf9b474dca12d3}{rpp::source::just}}(1).repeat().subscribe\_on(\mbox{\hyperlink{classrpp_1_1schedulers_1_1new__thread}{rpp::schedulers::new\_thread}}\{\});}
\DoxyCodeLine{\textcolor{keyword}{auto} s2 = \mbox{\hyperlink{group__creational__operators_gae8ac01568a0e057e97cf9b474dca12d3}{rpp::source::just}}(2).repeat().subscribe\_on(\mbox{\hyperlink{classrpp_1_1schedulers_1_1new__thread}{rpp::schedulers::new\_thread}}\{\});}
...
\item if you want to avoid it, you can convert your observable to dynamic\+: it forces to move observable to shared\+\_\+ptr, as a result, no any future copies/moves
...
To achieve better performance use {\ttfamily specific\+\_\+observable} while it is possible. Same for the argument of callback (for example, when you use {\ttfamily rpp\+::source\+::create})\+: use {\ttfamily const auto\&} for subscriber to avoid implicit conversion to dynamic subscriber.\hypertarget{advanced_autotoc_md2}{}\doxysubsection{Observers}\label{advanced_autotoc_md2}
By default, functional programming deals with immutable data and \char`\"{}pure functions\char`\"{}. Observer follow this principle, so, it can accept only const functions for callbacks.\hypertarget{advanced_autotoc_md3}{}\doxysubsection{Operators}\label{advanced_autotoc_md3}
...
\hypertarget{specific_vs_dynamic_autotoc_md21}{}\doxysubsection{Brief overview}\label{specific_vs_dynamic_autotoc_md21}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{specific_vs_dynamic_autotoc_md22}{}\doxysubsection{Reactive\+Plus\+Plus solution}\label{specific_vs_dynamic_autotoc_md22}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{specific_vs_dynamic_autotoc_md23}{}\doxysubsection{Examples}\label{specific_vs_dynamic_autotoc_md23}
...
\hypertarget{memory_model_autotoc_md15}{}\doxysubsection{Overview}\label{memory_model_autotoc_md15}
...
So, R\+PP provides ability to select strategy \char`\"{}how to deal with such a variables\char`\"{} via {\ttfamily rpp\+::memory\+\_\+model} enum.\hypertarget{memory_model_autotoc_md16}{}\doxysubsection{Examples}\label{memory_model_autotoc_md16}
For example, {\ttfamily rpp\+::source\+::just}


\begin{DoxyCode}{0}
\DoxyCodeLine{\mbox{\hyperlink{group__creational__operators_gae8ac01568a0e057e97cf9b474dca12d3}{rpp::source::just}}(my\_custom\_variable);}
\end{DoxyCode}
...
As a a result, users can select preferable way of handling of their types. \hypertarget{status}{}\doxysection{Implementation status}\label{status}
\hypertarget{status_autotoc_md5}{}\doxysubsection{Foundamentals}\label{status_autotoc_md5}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{status_autotoc_md6}{}\doxysubsection{Creating Observables}\label{status_autotoc_md6}
...
\end{DoxyItemize}\hypertarget{status_autotoc_md7}{}\doxysubsection{Operators}\label{status_autotoc_md7}
\hypertarget{status_autotoc_md8}{}\doxysubsubsection{Transforming}\label{status_autotoc_md8}

\begin{DoxyItemize}
\item \mbox{[}x\mbox{]} map
\end{DoxyItemize}\hypertarget{status_autotoc_md9}{}\doxysubsubsection{Filtering}\label{status_autotoc_md9}

\begin{DoxyItemize}
\item \mbox{[}x\mbox{]} filter
\item \mbox{[}x\mbox{]} take
\end{DoxyItemize}\hypertarget{status_autotoc_md10}{}\doxysubsubsection{Conditional}\label{status_autotoc_md10}

\begin{DoxyItemize}
\item \mbox{[}x\mbox{]} take\+\_\+while 
\end{DoxyItemize}\hypertarget{status_autotoc_md11}{}\doxysubsubsection{Combining}\label{status_autotoc_md11}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{status_autotoc_md12}{}\doxysubsubsection{Utility}\label{status_autotoc_md12}

\begin{DoxyItemize}
\item \mbox{[}x\mbox{]} observe\+\_\+on
\item \mbox{[}x\mbox{]} repeat
\item \mbox{[}x\mbox{]} subscribe\+\_\+on 
\end{DoxyItemize}\hypertarget{status_autotoc_md13}{}\doxysubsubsection{Connectable}\label{status_autotoc_md13}
...
\end{DoxyItemize}\hypertarget{status_autotoc_md14}{}\doxysubsection{Subjects}\label{status_autotoc_md14}
