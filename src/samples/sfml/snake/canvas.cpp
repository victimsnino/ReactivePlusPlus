...
sf::RectangleShape get_rectangle_at(Coordinates location, sf::Color color)
{
    ...
    box.setPosition(sf::Vector2f{ (s_cell_size+s_gap_size) * location.x, (s_cell_size + s_gap_size) * location.y });
...
{
    return {static_cast<uint32_t>(cols_count * (s_cell_size + s_gap_size)),
            static_cast<uint32_t>(rows_count * (s_cell_size + s_gap_size))};
}
