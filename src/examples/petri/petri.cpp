...
const Marking  m_start = {{'a', 4}, {'b', 2}, {'c', 0}, {'d', 0}};

const std::map<Transition, TransitionFunction> store = {{0, &foo}, {1, &bar}};

const auto& runTransition(Transition t)
{
    store.at(t)();            // calls the function associated with the transition
    return net.at(t).produce; // contains the post marking mutations.
}

std::pair<Marking, Transitions> mutateMarking(Marking&& m, const Places& produce)
{
    // Produce tokens
    for_each(produce, [&m](const auto& place) { m[place] += 1; });
    ...
    for (const auto& [transition, mutations] : net)
    {
        ...
        if (!c.empty() &&
            all_of(c, [&](const auto& place) { return m[place] >= count(c, place); }))
        {
            for_each(c, [&m](const auto& place) { m[place] -= 1; });
            transitions.push_back(transition);
        }
    }

    return std::make_pair(m, transitions);
}
...
int main()
{
    ...
    const Marking m_end         = {{'a', 0}, {'b', 2}, {'c', 0}, {'d', 2}};
    const auto    end_predicate = [=](const auto& m)
    { return !std::equal(m.begin(), m.end(), m_end.begin()); };
    ...
    transitions_subject.get_observable()
        ...
        .scan(m_start,
              [&transitions_subject](auto&& m, const auto& p)
              {
                  Transitions new_t;
                  std::tie(m, new_t) = mutateMarking(std::move(m), p);
                  for_each(new_t,
