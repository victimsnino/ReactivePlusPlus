 ...
 * Also *technically* you could create your observer via `make_lambda_observer` function, but it is not recommended too: it could disable some built-in optimizations and cause worser performance. <br>
 * Also it is **most probably** bad pattern and ivalid usage of RX if you want to keep/store observers as member variables/fields. Most probably you are doing something wrong IF you are not implementing custom observable/operator.
 *
