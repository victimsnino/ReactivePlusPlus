...
private:
    template<rpp::constraint::decayed_same_as<T> TT>
    subjects::publish_subject<Type>* deduce_subject(const rpp::constraint::observer auto& obs, const TT& val) const
    {
        ...
        if (inserted)
        {
            ...
            obs.on_next(rpp::grouped_observable_group_by<TKey, Type>{key, group_by_observable_strategy<Type>{itr->second, disposable}});
        }
...
struct group_by_t : public operators::details::template_operator_observable_strategy<group_by_observer_strategy, KeySelector, ValueSelector, KeyComparator> 
{
    ...
    using ResultValue = grouped_observable<utils::decayed_invoke_result_t<KeySelector, T>, utils::decayed_invoke_result_t<ValueSelector, T>, group_by_observable_strategy>;
};
}
...
{
    return details::group_by_t{std::forward<KeySelector>(key_selector), std::forward<ValueSelector>(value_selector), std::forward<KeyComparator>(comparator)}
}
} // namespace rpp::operators